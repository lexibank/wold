...
class Dataset(Base):
    ...
    def cmd_makecldf(self, args):
        ...
        languages = {
            r["pk"]: r["name"]
            for r in self.raw_dir.joinpath("db").read_csv("language.csv", dicts=True)
        }
        codes = {
            r["pk"]: r["name"]
            for r in self.raw_dir.joinpath("db").read_csv("identifier.csv", dicts=True)
            if r["type"] == "glottolog"
        }
        glottocodes = {
    ...
    def _schema(self, args):
        ...
        args.writer.cldf["FormTable", "Comment"].common_props[
            "dc:description"
        ] = "For more specific comments see 'comment_on_borrowed' and 'comment_on_word_form'"
        args.writer.cldf["FormTable", "Word_ID"].valueUrl = URITemplate(
        ...
        t = args.writer.cldf.add_component(
            "BorrowingTable",
            {
                ...
                "dc:description": "Whether a word was contributed directly (immediate) or indirectly (earlier), "
